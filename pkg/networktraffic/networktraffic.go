...
func (n *NetworkTraffic) Score(ctx context.Context, state *framework.CycleState, p *v1.Pod, nodeName string) (int64, *framework.Status) {
	// call the GetNodeBandwidthMeasure function from the Prometheus structure (prometheus.go) providing the node name.
	...
	// The call will return a Sample which holds the value in the Value field.
	return int64(nodeBandwidth.Value), nil //return the total bytes received by each node in a determined period of time
}
...
//do the normalization, the plugin needs to know the highest score among all nodes.
func (n *NetworkTraffic) NormalizeScore(ctx context.Context, state *framework.CycleState, pod *v1.Pod, scores framework.NodeScoreList) *framework.Status {
	var higherScore int64 //highest score among all nodes
	...
	//normalize the scores, the highest score will be 0 and the lowest will be 100
	// higher score means higher traffic, so we need to reverse the scores. Then, higher score for lower traffi, and lower score for higher traffic.
